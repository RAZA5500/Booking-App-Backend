...
import { BOOKING_STATUS, ROLES, config } from '../config/configuration';
import { PricingService } from '../domain/pricing.service';
import type { Booking, Favorite, Review, Role, User } from '../types/models';
import { HOTELS } from './hotels.seed';
...
const REVIEW_LINES: Array<[string, string]> = [
...
export class SeedService {
  ...
  seedIfEmpty({ force = false }: { force?: boolean } = {}): SeedSummary {
    ...
    // ---------------------------------------------------------------- users
    const users: User[] = DEMO_ACCOUNTS.map((account, index) => ({
    ...
    const soldOut = (hotelId: string, startOffset: number, nights: number) => {
      const hotel = HOTELS.find((h) => h.id === hotelId);
      if (!hotel) return;
    ...
    const favorites: Favorite[] = customers.slice(0, 6).flatMap((customer, i) =>
    ...
    this.store.replaceAll('hotels', structuredClone(HOTELS));
    this.store.replaceAll('bookings', bookings);
    this.store.replaceAll('reviews', reviews);
    this.store.replaceAll('favorites', favorites);
    this.store.replaceAll('audit', []);
